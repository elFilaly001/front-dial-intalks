  ...
  const columns: TableColumn<Network>[] = [
    ...
    },
    {
      ...
      cell(row) {
        ...
        return (
          <Badge
            className={`flex gap-1 items-center ${isPositive ? "bg-emerald-500/20" : "bg-red-500/20"} text-black px-3 py-2 rounded-md`}
          >
            <span
              className={`circle h-2 w-2 ${isPositive ? "bg-green-500" : "bg-red-500"} rounded-full`}
            ></span>
            {isPositive ? "+" : ""}
            {growth.toFixed(1)}%
          </Badge>
  ...
  return (
    <div>
      ...
      <DataTable
        columns={columns}
        data={
          source === "All Social Medias"
            ? networks
            : networks.filter(
                (n) => n.network.toLowerCase() === source.toLowerCase(),
              )
        }
      />
    </div>
