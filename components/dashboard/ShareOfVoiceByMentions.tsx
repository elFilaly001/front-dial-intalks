...
export default function ShareOfVoiceByMentionsDonut({ rows }: Props) {
  ...
  const brands =
    rows && rows.length > 0
      ? rows.map((r, i) => ({
          label: r.label,
          value: parseFollowersCount(r.followers),
  ...
  const positive =
    rows && rows.length > 0
      ? brands.map((b) => Math.round(b.value * 0.1))
      : defaultPositive;
  const negative =
    rows && rows.length > 0
      ? brands.map((b) => Math.round(b.value * 0.12))
      : defaultNegative;
  const neutral = brands.map(
    (b, i) => b.value - (positive[i] || 0) - (negative[i] || 0),
  );
                  ...
                  {posLegend.map((l) => (
                    <div key={l.label} className="flex items-center gap-2">
                      ...
                      <span className="truncate">
                        {l.count.toLocaleString()} ({l.percent.toFixed(2)}%)
                      </span>
                    </div>
                  ))}
                </div>
            ...
            {showInsight && (
              <div className="absolute bottom-full left-0 mb-2 p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-50 w-auto min-w-80 max-w-xl">
                <p className="text-sm text-gray-700 dark:text-gray-300">
                  Crédit du Maroc domine la part de voix avec 35 000 mentions,
                  soit près de 37,6 % du total, devançant Attijariwafa Bank (25
                  000) et CIH Bank (18 000). La répartition des sentiments
                  révèle une perception globalement positive, portée par une
                  communication digitale active et une présence renforcée sur
                  les réseaux sociaux. Cette dynamique témoigne d'un
                  positionnement concurrentiel solide sur le marché bancaire
                  marocain.
                </p>
              </div>
            )}
